...
echo "Запуск исправленного MCP HTTP сервера для Wiki.js на порту $PORT с базовым URL $WIKIJS_BASE_URL"
node fixed_mcp_http_server.js > fixed_server.log 2>&1 &

# Сохраняем PID
echo $! > server.pid
echo "Исправленный MCP HTTP сервер запущен, PID: $(cat server.pid)"
